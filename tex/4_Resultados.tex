...
A função de loss utilizada no Modelo 6 é \equacao{loss}{Ke^{(-0.07r)}Q} onde $K = 3.5 \times 10^{-7}$ e $Q$ é a quantidade de bits 1 no latente binarizado. Com esta função de \textit{loss} há uma penalização na ocorrência do bit 1 o que leva a uma diminuição da entropia do latente binarizado. Espera-se que o codificador de entropia seja mais eficaz dessa forma.

Os resultados são mostrados nas~\refFigs{_mean_plot_loss_psnr}{_mean_plot_loss_msssim}. Foram feitos alguns testes usando \textit{10-fold cross validation}. A média obtida pelo modelo 5 foi 30.23 dB de \acrshort{PSNR} e 0.86340 de \acrshort{SSIM} à uma taxa de 0.71 bpp. A média obtida pelo modelo 6 obteve 30.31 dB de \acrshort{PSNR} e 0.86498 de \acrshort{SSIM} à uma taxa de 0.71 bpp.

\figura[!htbp]{_mean_plot_loss_psnr}{Comparação do Modelo 5 e 6 para a métrica \acrshort{MSE}}{_mean_plot_loss_psnr}{width=0.7\textwidth}
\figura[!htbp]{_mean_plot_loss_ssim}{Comparação do Modelo 5 e 6 para a métrica \acrshort{SSIM}}{_mean_plot_loss_ssim}{width=0.7\textwidth}
\figura[!htbp]{_mean_plot_loss_msssim}{Comparação do Modelo 5 e 6 para a métrica \acrshort{MS-SSIM}}{_mean_plot_loss_msssim}{width=0.7\textwidth}
...
\subsection{Modelos 5 e 6}
Na~\refFig{plot_gzip_models_5_6} é possível notar um ganho superior ao ganho mostrado na~\refFig{gain_gzip_mod4}. Isso pode ter ocorrido devido ao menor número de níveis de resíduos existentes para esse modelo, o que leva à formação de um \textit{bitstream} menos complexo para o modelo como um todo. Pode-se considerar que a diferença no ganho do \textbf{GZIP} para os Modelos 5 e 6 é inexistente.

\figura[!htbp]{plot_gzip_models_5_6}{Ganho percentual médio na taxa por nível ao usar o codificador de entropia
gzip nos bitstreams de cada nível para a base Kodak~\cite{kodak}}{plot_gzip_models_5_6.pdf}{width=0.7\textwidth}